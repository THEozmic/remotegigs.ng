...
import { apiGetJobs } from "../actions";
...
export class Jobs extends Component {
  constructor(props) {
    super(props);
    // jobs.reduce(job => (job.isExpanded = false));
    this.state = {
      jobs: []
    };
    // this.props.apiGetJobs();
  }
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps !== this.props) {
      this.setState({
        jobs: nextProps.jobs.reduce(job => (job.isExpanded = false))
      });
    }
  }
  ...
  handleJobExpand(index) {
    ...
    console.log("Job: ", this.props.jobs[index]);
  }

  returnJobDescription(index) {
    return { __html: jobs[index].description };
  }
            ...
            <div key={index}>
              <div className="job-item">
                <div
                  ...
                  onClick={() => this.handleJobExpand(index)}
                >
                  {job.position}
                  <div className="job-date">{moment(job.date).fromNow()}</div>
                  <div className="tags">
                ...
                <div
                  className={`job-details ${job.isExpanded ? " visible" : ""}`}
                >
                  <div
                    className="job-description"
                    dangerouslySetInnerHTML={this.returnJobDescription(index)}
                  />
                  <div className="apply-btn-container">
                    <a
                      className="apply-btn btn btn-primary btn-rounded"
                      href={job.url}
                      target="_blank"
                    >
                      Apply
              ...
              <button
                className="job-btn btn btn-primary"
                onClick={() => this.handleJobExpand(index)}
              >
                {job.isExpanded ? "Collapse" : "Expand"}
              </button>
            </div>
          );
        })}
